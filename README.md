...
Роль user создает .gitignore внутри удаленного проекта и добавляет туда .env, .env.* и исключение для .env.example.

Перед git add . роль также выполняет git rm --cached --ignore-unmatch .env. Это защищает от ситуации, когда deploy_bot уже положил .env с токенами в проект, а затем user запускается повторно.

Если секреты уже были запушены в GitHub, токены нужно перевыпустить: удаления файла из нового коммита недостаточно, потому что старые значения остаются в истории репозитория.
